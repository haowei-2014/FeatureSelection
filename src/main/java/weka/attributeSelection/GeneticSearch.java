   ...
   public int[] search (ASEvaluation ASEval, Instances data)
    ...
    for (int i=1;i<=m_maxGenerations;i++) {
      ...
      if ((i == m_maxGenerations) || 
        ...
        System.out.println("Generation " + i + " is produced.");  // commented by Hao
        if (converged == true) {
          break;
        }
      }
  ...
  private void evaluatePopulation (SubsetEvaluator ASEvaluator)
    ...
    for (i=0;i<m_popSize;i++) {
      ...
      if (m_lookupTable.containsKey(m_population[i]
        ...
        int countFeatures = countFeatures(m_population[i].getChromosome()); // Hao
//        merit =  Math.pow(merit, 7) * countFeatures;   // Hao
//        merit = merit / (1 + 1/countFeatures);  // Hao
        merit = adaptMerit(merit, countFeatures);  // Hao
  ...
  private double adaptMerit (double merit, int nbFeatures) {
	  double accuracy = 1 + merit;
//	  double weight = 0.9;
//	  merit = weight * accuracy + (1-weight) * m_numAttribs / nbFeatures;
//	  return merit;
	  return accuracy;
  } 
