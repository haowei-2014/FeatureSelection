...
import Hao.AttributeSelectionNB;
import Hao.AttributeSelectionNB_Me;
...
public class GreedyStepwise extends ASSearch implements RankedOutputSearch,
	...
	// write the best_merit to a file
	public File file;
	...
	// by Hao. This is to write the best_merit of each iteration to a file
	public void writeMerit(double best_merit) {
		try {
			file = new File(AttributeSelectionNB.path 
							+ AttributeSelectionNB.file + "_best_merit.txt");
			if (!file.exists()) {
				file.createNewFile();
			}
			FileWriter fw = new FileWriter(file.getAbsoluteFile(), true);
			BufferedWriter bw = new BufferedWriter(fw);
			String str_best_merit = String.valueOf(1.0 - best_merit*(-1));
			bw.write(str_best_merit);
			bw.newLine();
			bw.close();
		} catch (IOException e) {
			e.printStackTrace();
	...
	public int[] search(ASEvaluation ASEval, Instances data) throws Exception {
		...
		while (!done) {
			...
			// begin modification by Hao
			System.out.println(best_merit);
//			writeMerit(best_merit);
			temp_best = 1.5 * best_merit;	
			// stop when the selected feature reach a limit
			double nbFeatureStop = m_numAttribs * AttributeSelectionNB_Me.ratioNbFeatures;			
			if (m_best_group.cardinality() == (int)nbFeatureStop) {
		//	if (m_best_group.cardinality() == AttributeSelectionNB.nbFeatureLimit) {
				return attributeList(m_best_group);
			}
			System.out.println(m_best_group.toString());
			// end modification
			if (m_doRank) {
